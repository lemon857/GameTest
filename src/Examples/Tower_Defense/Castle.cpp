...
Castle::Castle(glm::vec3 initPos, unsigned int hp, Cube* model, std::shared_ptr<RenderEngine::Material> pMaterial)
	: m_hp(hp)
	, m_model(std::move(model))
	, m_isDestroyed(false)
	, m_bar(new HealthBar(pMaterial, initPos + glm::vec3(0.f, 2.5f, 0.f), 25, 2, hp, glm::vec3(1.f), glm::vec3(1.f, 0.f, 0.f)))
{
	model->addComponent<Transform>()->set_position(initPos);
}
